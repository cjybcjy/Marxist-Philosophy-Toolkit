...
理论框架层。用于吸收《马克思主义基本原理概论》这类教材的优点：把原著中分散的概念整理成可检索、可调度、可维护的体系。

它的定位是：

```text
framework-synthesis：负责目录、分类、概念关系、标准教学顺序
primary-text：负责原文依据和历史任务
method-template：负责可重复执行的分析流程
current-fact：负责现代问题的事实判断
```

也就是说，教材层可以帮助 Skill 判断一个问题属于：
...
但教材不能替代马恩列毛邓原著，也不能替代对现代产业、政策、市场、组织的事实调查。

### `method-templates.md`

方法模板层。用于把原著和教材中分散的方法抽成可调用流程。

当前包括：

| 模板 | 用途 |
| --- | --- |
| 矛盾分析法模板 | 判断主要矛盾、矛盾主要方面、转化条件和行动重点 |
| 实践检验分析法模板 | 把判断变成可反馈的小实验 |
| 历史唯物主义分析模板 | 分析生产力、生产关系、经济基础、上层建筑、利益结构 |
| 发展趋势 / 否定分析模板 | 分析旧形式、新矛盾、量变、质变、保留与扬弃 |

模板不直接给结论，只规定推理步骤。每次仍然要填入现实对象、事实缺口、来源边界和实践指标。
...
人物源文索引。它是 `source-map.md` 的下一层，不是全文库，也不是引文库。

当前包含：

| 文件 | 作用 |
| --- | --- |
| `source-index/marx-engels.md` | 马克思、恩格斯本人著作索引：资本、阶级、历史唯物主义、国家形式、过渡等 |
| `source-index/lenin.md` | 列宁本人著作索引：帝国主义、国家机器、组织、策略转换、新经济政策、官僚问题等 |
| `source-index/mao.md` | 毛泽东本人著作索引：矛盾、实践、调查研究、群众路线、根据地、统一战线、建设期矛盾等 |
| `source-index/deng.md` | 邓小平本人讲话/谈话/文集索引：实践标准、改革开放、发展阶段、生产力、试点、南方谈话等 |
...
一个方法如果只是一句口号、只来自二手解释、不能生成新的调查或实践方案，就不能升级为稳定方法模块。

### `cases/figure-case-calibration.md`

案例校准层。用于把人物方法应用到现代问题时，检查“相似机制”和“不同条件”。

例如：

```text
不是说今天的问题等于井冈山、NEP 或南方谈话，
而是看它们是否共享某个机制：
弱势方根据地、阶段转换、策略退却、制度试点、实践标准。
```

它防止 Agent 把历史案例当成励志故事，也防止把商业类比当成理论证明。
